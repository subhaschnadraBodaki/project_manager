...
export default function TableToolbar({projectId,label}) {
    ...
    // };
        return (<>
            ...
            <React.Fragment>
                <button className="bg-blue-900 px-5 py-3 text-sm shadow-sm font-medium tracking-wider text-white rounded-md hover:shadow-lg  flex"  onClick={()=> setTaskIsOpen(true)}  ><PlusIcon className="h-5  w-5" /> {label}</button>
            </React.Fragment>
        </>
        )
      }
