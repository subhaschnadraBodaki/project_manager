...
export default function addproject({data}) {
    ...
    const addProjectForm =keycloak.authenticated?( <ProjectForm account={data}/>):(<> <span>You have been logged out click here to login again</span> <br/> < button type="button" onClick={() => keycloak.login()}>
    Login
    </button></>)
    return (
        <>
