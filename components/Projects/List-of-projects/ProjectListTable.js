...
// })
    ...
    const statusBodyTemplate = (data) => {
        if (data.project_status == null) {
            ...
            return <span className={`text-sm text-${data.project_status.toLowerCase()}-500 bg-${data.project_status.toLowerCase()}-100 rounded-sm bg-cover p-1`} >{data.project_status}</span>;
        }
    }
