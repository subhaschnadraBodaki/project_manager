...
function EditResourceReq({ projectId,editData}) {
...
const dropdownRequester= [
  ...
  const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/resource_requests?id=eq.${editData.id}`;
  ...
  const mutation = useMutation(addTask, {
    ...
    onSettled: (data, error) => {
      toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Resource Request Updated', life: 3000 });
...
export default EditResourceReq;
