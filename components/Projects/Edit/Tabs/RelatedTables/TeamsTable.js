...
export default function TeamsTable({ projectsData }) {
    ...
    if (projectsData[0] == null || projectsData[0] === undefined || projectsData[0].project_team_member[0] == null || projectsData[0].project_team_member[0] === undefined) {
      return ( 
    <div>
      <TableHeader projectId={projectId} projectName={projectName} label='Add Team' formType='Add Team'/>
...
const [teamsData,setTeamsData] =useState(projectsData[0].project_team_member)

const deleteProduct = (deleteData) => {
        let  _teamsData = teamsData.filter(val => val.id !== deleteData.id);
        setTeamsData(_teamsData)
        setDeleteItemConfirm(false)
        //  console.log(rowData)
         toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Team Deleted', life: 3000 });

        const url =  `${process.env.NEXT_PUBLIC_SUPABASE_URL}/project_team_members?id=eq.${deleteData.id}` 
      axios.delete(url,
            {
            headers:{
    ...
    const ActionButton = (rowData) => {
         ...
         return (
            ...
            <div className="grid grid-cols-3">
                ...
                <h2 className="ModalHeading">Edit Team
                </h2>
                </div>
             ...
             <div>
              <EditTeam projectId={projectId} editData={editData} />
