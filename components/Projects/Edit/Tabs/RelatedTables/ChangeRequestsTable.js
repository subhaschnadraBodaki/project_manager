...
import EditChangeReq from '../../EditForms/EditChangeReq'
import { Toast } from 'primereact/toast';

export default function ChangeRequestsTable({projectsData}) {
    ...
    if (projectsData[0] == null || projectsData[0] === undefined || projectsData[0].project_change_request[0] == null || projectsData[0].project_change_request[0] === undefined) {
           return ( 
    <div>
      <TableHeader projectId={projectId} projectName={projectName} label='Add Change Req.' formType='AddChangeReq'/>
     ...
     {
const [changeRequest,setChangeRequest] =useState(projectsData[0].project_change_request)


    // ----------------------------delete task from database and table------
   const deleteProduct = (deleteData) => {
        let  _changeRequest = changeRequest.filter(val => val.id !== deleteData.id);
        setChangeRequest( _changeRequest)
        ...
        const url =  `${process.env.NEXT_PUBLIC_SUPABASE_URL}/change_requests?id=eq.${deleteData.id}` 
      axios.delete(url,
            {
            headers:{
    ...
    const ActionButton = (rowData) => {
         ...
         return (
            ...
            <div className="grid grid-cols-3">
                ...
                <h2 className="ModalHeading">Edit Change Req.
                </h2>
                </div>
             ...
             <div>
              <EditChangeReq projectId={projectId} editData={editData} />
...
//    ==========================================================
    const columns = [
        ...
        {field:"change_request_priority"  , header:"Resource Req."}
    ]
const dynamicColumns = columns.map((col)=> {
    ...
    return (
        ...
        <DataTable value={changeRequest} className="p-datatable-sm"  resizableColumns columnResizeMode="expand">
                     {dynamicColumns}
