...
import EditCommunicationDetails from "../../RelatedTables/Edit/EditCommunicationDetails";

const CommunicationDetailsData = ({ employeeData, phoneType }) => {
  ...
  const [editCommunicationDetails, setEditCommunicationDetails] =
    useState(null);
  ...
  if (
    ...
    return (
      <div>
        <TableToolbar
          ...
          formType="AddCommunicationDetails"
          phoneType={phoneType}
        />
    ...
    const [communicationDetailsData, setCommunicationDetailsData] = useState(
      employeeData[0].communication_details
    );

    const deleteCommunicationDetails = (deleteData) => {
      let _communicationDetailsData = communicationDetailsData.filter(
        (val) => val.id !== deleteData.id
      );
      setCommunicationDetailsData(_communicationDetailsData);
      setDeleteItemConfirm(false);
      // console.log(rowData)
      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "details  Deleted",
        life: 3000,
      });

      const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/communication_details?id=eq.${deleteData.id}`;
      axios.delete(url, {
        headers: {
    ...
    const edit = (rowData) => {
      setEditCommunicationDetails(rowData);
      setModalIsOpen(true);
    };
        ...
        <React.Fragment>
          ...
          >
            ...
            <EditCommunicationDetails
              employeeId={employeeId}
              editCommunicationDetails={editCommunicationDetails}
              phoneType={phoneType}
            />
          ...
          >
            ...
            <div className="text-right md:mt-2">
              ...
              <Button
                ...
                icon="pi pi-check"
                className="p-button-text"
                onClick={() => deleteCommunicationDetails(deleteData)}
              />
            </div>
          </Modal>
    ...
    return (
      <div>
        ...
        <DataTable
          value={communicationDetailsData}
